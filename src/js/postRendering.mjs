...
export async function fetchPosts(type, user = null) {
  ...
  try {
    if (type == "all") {
        ...
        const response = await fetch(`https://facebok-2q7r.onrender.com/users/${user.sub}`, options);
        const userData = await response.json();
        const groups = userData.groups;

        for (var i = 0; i < groups.length; i++) {
          const result = await fetch("https://facebok-2q7r.onrender.com/posts/" + groups[i]);
        ...
        const response = await fetch(`https://facebok-2q7r.onrender.com/posts/user/${user.sub}`, options);
